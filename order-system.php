...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $page === 'create-order') {
    // Enhanced validation
    $required_fields = ['order_name', 'category', 'budget', 'description', 'completion_date'];
    $errors = [];
    
    foreach ($required_fields as $field) {
        if (empty($_POST[$field])) {
            $errors[] = "Поле " . getFieldLabel($field) . " обязательно для заполнения";
        }
    }
    ...
    if (empty($errors)) {
        ...
        $technical_requirements = htmlspecialchars($_POST['technical_requirements'] ?? '');
        
        // Store in session with timestamp
        $_SESSION['form_data'] = $_POST;
        $_SESSION['form_timestamp'] = date('Y-m-d H:i:s');
        $_SESSION['success_message'] = 'Заказ успешно размещен! ID: ' . uniqid();
        
        // Redirect to prevent form resubmission
        header('Location: order-system.php?page=create-order&success=1');
        exit();
    } else {
...
$show_errors = isset($_SESSION['form_errors']);
if ($show_errors) {
...
function getFieldLabel($field) {
    $labels = [
        ...
        'completion_date' => 'Срок выполнения'
    ];
